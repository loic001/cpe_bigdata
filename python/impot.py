...
def myPCA(df, clusters=None):
    ...
    if isinstance(clusters, np.ndarray):
        for clust in set(clusters):
            colors = list('bgrcmyk')
            plt.scatter(dat['PC-0'][clusters == clust], dat['PC-1'
                        ][clusters == clust], c=colors[clust])
    else:
        plt.scatter(dat['PC-0'], dat['PC-1'])
        for i, txt in enumerate(df_norm.index.get_values()):
...
...."""
    ...
    for pair in combinations(labels, 2):
        ...
        circle = Circle((labels.get_loc(pair[0]),
                        labels.get_loc(pair[1])), radius=0.4)
        circle.set_edgecolor('black')
        circle.set_facecolor(clrmap(np.abs(corr)))
        mirrorCircle = Circle((labels.get_loc(pair[1]),
                              labels.get_loc(pair[0])), radius=0.4)
...
def myAFC(df):
    ...
    c = myorngCA.CA(df.values, labelR=df.index.tolist(),
                    labelC=df.columns.tolist())
    c.Biplot()
...
if __name__ == '__main__':
    ...
    df = getAllData().drop(['patrimoine_total', 'stairs', 'sup'], axis=1)
